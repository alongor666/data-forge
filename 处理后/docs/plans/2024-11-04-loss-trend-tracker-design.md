...
### 核心目标
通过3层渐进式下钻,定位最严重且影响最大的赔付率问题,并追踪其在5周内的演变趋势。

### 架构定位
- **输入**: 5周CSV数据文件(第40-44周)
- **处理**: 独立运行,不修改现有周报系统
- **输出**: 独立报告 `赔付率趋势分析报告_第XX周.md`

### 与现有技能的关系
- **复用** `insurance-data-loader` - 数据加载逻辑
- **复用** `insurance-kpi-calculator` - 赔付率计算方法
- **独立** 不影响现有周报生成流程
